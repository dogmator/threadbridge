...
- optional idempotency key used for account-scoped publication recovery;
- projection state;
...
The optional `idempotency_key` remains indexed for projection recovery but is not globally unique:
the same client-chosen key may legitimately be used by different connected accounts. Durable
operation identity owns account-scoped uniqueness instead.
...
Stores durable identity and lifecycle state for a reply publication:
...
- request fingerprint;
- status;
- optional published comment identifier;
- optional last failure code;
- creation and update timestamps.

Supported states are `pending`, `published`, `retryable_failed`, `failed`, and `indeterminate`.
A published operation references exactly one stored comment; every failure-state operation records
...
2. loads the parent comment, post, account, and selected platform adapter;
3. creates or loads the durable operation identified by account and idempotency key;
4. rejects a mismatched parent or request fingerprint as an idempotency conflict;
5. replays a published operation, or completes an operation when its reply was stored before an
   interrupted final status update;
6. replays terminal and indeterminate outcomes without another provider call;
7. refuses an unsafe repeat of a pending operation when the provider has no publication-idempotency
   guarantee;
8. invokes the external platform outside every PostgreSQL transaction and lock;
9. saves a confirmed reply in a short PostgreSQL transaction;
10. marks the operation as published only after the reply is stored, or records the classified
    failure state;
11. returns the created or existing resource.

The current implementation never retries an indeterminate operation. An existing pending operation
reaches the provider again only when the selected adapter declares native publication idempotency.
A `retryable_failed` operation may be retried because that classification asserts that the adapter
can establish that no external write took effect.

### Scope of the idempotency guarantee

Two guarantees are involved, and only one of them belongs to this application.

The *local* guarantee is owned by ThreadBridge and is account-scoped: at most one durable
operation exists for `(account_id, idempotency_key)`. Its stored parent and request fingerprint make
identical requests replayable and mismatched reuse a conflict. The comment projection keeps the key
only for recovery and resolves it together with the owning account. Short transactions take an
account-and-key advisory lock while creating the operation or converging a confirmed reply; no lock
survives an external call.

The *external* guarantee — that one key produces at most one reply on the social platform — can only
be given by the platform. The application deliberately holds no database transaction and no lock
across the external call, so two concurrent requests sharing a key may both reach an adapter that
supports safe publication replay. The idempotency key is passed to the adapter for exactly that
reason: an adapter forwards it as the provider's idempotency token, or otherwise relies on an
equivalent provider guarantee. The full demo adapter deduplicates on its own side and is therefore
externally idempotent. An adapter without that guarantee is not retried from an existing pending
operation; the operation becomes indeterminate instead of risking a duplicate external write.
...
interface ReplyPublicationOperationRepository {
    begin(
        input: BeginReplyPublicationInput,
    ): Promise<BeginReplyPublicationResult>;
    markPublished(operationId: string, commentId: CommentId): Promise<void>;
    markFailed(
        operationId: string,
        status: 'retryable_failed' | 'failed' | 'indeterminate',
...
The comment persistence port has no page-retrieval method, because the platform owns published
content: a page is read from the platform and then projected locally, never served back from the
projection. Its account-scoped idempotency lookup exists only to replay or recover reply publication.
Deciding which platform a request belongs to is a separate and deliberately narrow responsibility,
so one port resolves the platform binding of a published post and another resolves the reply context
of a comment. Neither is a generic repository, and neither is reused as one.
...
Reply persistence reports whether it created a row or converged on one an earlier request had
already stored, so the transport can answer `201 Created` or `200 OK` without inspecting persistence
details. The operation repository separately owns durable request identity and lifecycle state.

Every implemented write is owned by its adapter and is atomic as seen by the application: importing
a page upserts on external identity, beginning an operation uses one short transaction before the
external call, reply persistence uses another short transaction only after the platform has answered,
and operation completion is a separate bounded update. No application-visible transaction manager
exists, because atomicity is required within each adapter operation rather than across the provider
call. One is introduced only when behavior requires it.
...
- incremental polling;
- reconciliation;
- manual reconciliation and operator tooling for indeterminate operations;
